...
function addSubscription(prevVersion)
{
  ...
  /* ADSIDEBAR - don't add acceptable ads subscription 
  ...
  */
  ...
  // ADSIDEBAR - easylist adsidebar supplement subscription
  var subscription_adsidebar = Subscription.fromURL(Prefs.subscriptions_adsidebarurl);
  if (subscription_adsidebar)
  {
    subscription_adsidebar.title = "Adsidebar Easylist Supplement";
    FilterStorage.addSubscription(subscription_adsidebar);
    if (subscription_adsidebar instanceof DownloadableSubscription && !subscription_adsidebar.lastDownload)
      Synchronizer.execute(subscription_adsidebar);
  }
...
ext.onMessage.addListener(function (msg, sender, sendResponse)
{
  switch (msg.type)
  {
    ...
    case "forward":
      ...
      break;
    case "adsidebar-allow-web-requests":
      var frame1 = ext.getFrame(sender.page.id, 0);
      frame1.adsidebar_allowRequests = true;
      sendResponse({type : "adsidebar-allow-web-requests-response"});
      break;
    case "adsidebar-disallow-web-requests":
      var frame2 = ext.getFrame(sender.page.id, 0);
      frame2.adsidebar_allowRequests = false;
      break;
      
    case "adsidebar-get-url-info":
      
      let match = defaultMatcher.matchesAdsidebarList(msg.location, RegExpFilter.typeMap.DOCUMENT,  null, null, null, null);

      if (match) {
        sendResponse(
          { foundMatch: true,
            refreshgpt : match.refreshgpt,
            runlocalscripts : match.runlocalscripts,
            overridejqueryready : match.overridejqueryready});
      } else {
        sendResponse(
          { foundMatch: false,
            refreshgpt : null,
    ...
    case "adsidebar-get-child-prefs":
        sendResponse(
